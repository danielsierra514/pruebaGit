...
function listarImagenesProyecto() {
	...
	$.each(imagenesProyecto, function(key, imagen) {
		$("#foto-carrusel tr").append("<td><div class='cajonImagen'><a href='#' onclick=eliminarImagenProyecto('" + key + "')><img class='delete' src='archivos/icons/delete.png'></a><img src='archivos/fotos/" + imagen + ".png'></div></td>");
	});
}
/********************************************************/
var proyectosCuenta = [];
...
function llamarProyectosAgente() {
	borrarProyectos();
...
function crearObjetoProyecto(item) { /*PDF*/
	...
	editarProyecto(marker, mapaAgente);
}
